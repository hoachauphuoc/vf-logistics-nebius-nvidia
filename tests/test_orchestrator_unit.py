...
from vf_logistics.store import MemoryStore, utcnow
...
def _fresh_patches():
    ...
    return {
        "vf_logistics.orchestrator.analyze_shipment": AsyncMock(return_value=_agent_response(risk=25)),
        "vf_logistics.orchestrator.screen_shipment": AsyncMock(return_value=_compliance_response()),
        "vf_logistics.orchestrator.investigate_case": AsyncMock(return_value=_agent_response(risk=75)),
        "vf_logistics.orchestrator.extract_shipment": AsyncMock(),
        "vf_logistics.orchestrator.governance.execute": AsyncMock(return_value={"status": "done", "detail": {}}),
        "vf_logistics.orchestrator.governance.agent_readiness": AsyncMock(return_value={"state": "ACTIVE"}),
        "vf_logistics.orchestrator.document_store.archive": AsyncMock(return_value={"archived": True, "uri": "gs://test"}),
        "vf_logistics.orchestrator.document_render.render_bill_of_lading": MagicMock(return_value=b"%PDF"),
        "vf_logistics.orchestrator.model_armor.screen": AsyncMock(return_value={"blocked": False}),
        "vf_logistics.orchestrator.model_armor.extract_pdf_text": MagicMock(return_value=("text", None)),
        "vf_logistics.orchestrator.shipper_registry.enrich": MagicMock(return_value={"status": "unknown"}),
        "vf_logistics.orchestrator.untrusted.sanitise_shipment": MagicMock(
            side_effect=lambda raw: {
                "shipment": raw, "extraction_notes": [], "extraction_confidence": 0.95,
                "dropped_fields": [], "forbidden_fields_attempted": [],
            }
        ),
        "vf_logistics.orchestrator.untrusted.screen_text": MagicMock(return_value={"blocked": False, "findings": []}),
        "vf_logistics.orchestrator.untrusted.searchable_text": MagicMock(return_value="searchable"),
    }
...
class TestOrchestratorIngest(unittest.TestCase):
    def setUp(self):
        ...
        self.store_patcher = patch("vf_logistics.orchestrator.get_store", return_value=self.store)
...
class TestOrchestratorPreFilter(unittest.TestCase):
    ...
    @patch("vf_logistics.orchestrator.verifier.validate", return_value=_validation_result(skip_ai=True))
    def test_whitelist_fast_path_skips_ai(self, mock_validate):
        ...
        self.mocks["vf_logistics.orchestrator.analyze_shipment"].assert_not_called()

    @patch("vf_logistics.orchestrator.verifier.validate", return_value=_validation_result(
...
class TestOrchestratorAIPath(unittest.TestCase):
    ...
    @patch("vf_logistics.orchestrator.verifier.validate", return_value=_validation_result(risk_floor=10))
    @patch("vf_logistics.orchestrator.verifier.reconcile", return_value={
    ...
    def test_advance_ingested_to_specialists_done(self, mock_reconcile, mock_validate):
        ...
        self.mocks["vf_logistics.orchestrator.analyze_shipment"].assert_called_once()
        self.mocks["vf_logistics.orchestrator.screen_shipment"].assert_called_once()
    ...
    @patch("vf_logistics.orchestrator.verifier.validate", return_value=_validation_result(risk_floor=60))
    @patch("vf_logistics.orchestrator.verifier.reconcile", return_value={
        "effective_risk": 80, "model_risk": 80, "risk_floor": 60, "source": "agent score",
        "score_disputed": False, "auto_clear_permitted": False,
    })
    @patch("vf_logistics.orchestrator.verifier.check_exposure_claim", return_value=None)
    def test_high_risk_escalates(self, mock_exposure, mock_reconcile, mock_validate):
        from vf_logistics import orchestrator
        self.mocks["vf_logistics.orchestrator.screen_shipment"].return_value = _compliance_response("REVIEW_REQUIRED")
...
class TestTools(unittest.TestCase):
    def setUp(self):
        self.store = _make_store()
        self.store_patcher = patch("vf_logistics.tools.get_store", return_value=self.store)
        self.store_patcher.start()
    ...
    @patch("vf_logistics.tools.NOTIFY_WEBHOOK_URL", "")
...
class TestAgentModules(unittest.TestCase):
    @patch("vf_logistics.agents.fraud_detection_agent.nebius_client.complete_json",
           new_callable=AsyncMock,
           return_value=('{"risk_score": 25, "findings": [], "recommendations": []}', 100, 50))
    def test_fraud_agent_returns_envelope(self, mock_complete):
        from vf_logistics.agents.fraud_detection_agent import analyze_shipment
