...
| `auto_debate` | `debate_agent.py` | Super | CONFIRM or DISAGREE with Nano, with an adjusted score |
...
| `nvidia/nemotron-3-super-120b-a12b` | 0.30 | 0.90 | investigation, debate |
| `nvidia/Nemotron-3-Ultra-550b-a55b` | 1.00 | 3.00 | nothing by default; opt-in only |
...
`infra/firestore.indexes.json` defines 21 composite indexes -- 14 on `cases`, 4 on
...
Ten of the `cases` indexes exist for `sum_rollups` alone. Their absence was not a slow
...
and the operations board went blank. Five of the ten are the windowed variants carrying
`created_at`, needed only once a billing period is requested -- so a deployment missing
those keeps serving the dashboard and fails on the first invoice.
...
| | |
| --- | --- |
| Backend | `vf-logistics`, `asia-southeast1` |
| Console | `vf-console`, revision `vf-console-00013-v49` |
| Store | Firestore native, `STORE_BACKEND=firestore` |
| Worker | `WORKER_MODE=ondemand` |
| Text model | `nvidia/NVIDIA-Nemotron-3-Nano-30B-A3B` |
